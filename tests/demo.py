            ...
            "config": {
                ...
                "steps": [
                    ...
                    },
                    {
                        "type": "text_to_image",
                        "model": "flux_dev",
                        "params": {"prompt": "{{step_1.output}}", "num_images": 2},
                    },
                ],
            },
        },
...
def demo_yaml_loading(manager):
    ...
    yaml_search_paths = ["input/pipelines/", "input/", ".", "examples/"]
...
def demo_console_commands():
    ...
    commands = [
        ...
        (
            "ai-content-pipeline run-chain --config pipeline.yaml",
        ...
        (
            "ai-content-pipeline create-speech --text 'Hello' --voice Rachel",
            "Create speech audio",
        ),
...
def interactive_demo():
    ...
    for i, (title, demo_func) in enumerate(demos, 1):
        ...
        response = (
            input(f"Ready to see '{title}' demo? (Enter to continue, 'q' to quit): ")
            .strip()
            .lower()
        )
        if response == "q":
