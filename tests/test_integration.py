...
def test_package_installation():
    ...
    try:
        ...
        from packages.core.ai_content_pipeline.ai_content_pipeline.pipeline.manager import (
            AIPipelineManager,
        )
...
def test_console_scripts():
    ...
    try:
        ...
        if result.returncode == 0 and "AI Content Pipeline" in result.stdout:
            ...
            print("❌ 'ai-content-pipeline' command failed")
            if result.stderr:
    ...
    try:
        result = subprocess.run(
            ["aicp", "--help"], capture_output=True, text=True, timeout=10
        )
        scripts_tested += 1
        if result.returncode == 0 and "AI Content Pipeline" in result.stdout:
    ...
    try:
        ...
        if result.returncode == 0 and (
            "Available" in result.stdout or "Supported Models" in result.stdout
        ):
            print("✅ 'list-models' command working")
            scripts_passed += 1
        else:
            print("❌ 'list-models' command failed")
    except Exception as e:
...
def test_parallel_execution():
    ...
    try:
        ...
        if chain.steps:
            ...
            print("✅ Parallel execution environment variable is set")
        else:
