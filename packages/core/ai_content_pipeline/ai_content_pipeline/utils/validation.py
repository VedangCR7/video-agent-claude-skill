...
def validate_image_dimensions(width: int, height: int, max_width: int = 2048, max_height: int = 2048) -> tuple[bool, str]:
    ...
    if aspect_ratio > 10:  # Max 10:1 aspect ratio (FIXED: was incorrectly rejecting valid ratios)
        return False, f"Aspect ratio too extreme ({aspect_ratio:.1f}:1). Maximum: 10:1"
    
        # Check aspect ratio is reasonable (not too extreme)
        aspect_ratio = max(width, height) / min(width, height)
        if aspect_ratio > 10:  # Allow up to 10:1 aspect ratio
            return False, f"Aspect ratio too extreme ({aspect_ratio:.1f}:1). Maximum: 10:1"
        ...
        aspect_ratio = max(width, height) / min(width, height)
        if aspect_ratio > 10:
            return False, f"Aspect ratio {aspect_ratio:.1f}:1 too extreme (max 10:1)"
...
def validate_processing_parameters(params: dict) -> tuple[bool, str]:
    """
    Validate processing parameters dictionary with comprehensive checks.
    ...
    if not params:
        return False, "Parameters dictionary cannot be empty"

    # Validate common parameter types
    if 'model' in params:
        model = params['model']
        if not isinstance(model, str) or not model.strip():
            return False, "Model parameter must be a non-empty string"
        if len(model) > 100:
    ...
    if 'duration' in params:
        try:
            ...
            if duration > 600:  # 10 minutes max
                return False, "Duration too long (max 600 seconds)"
        except (ValueError, TypeError):
            return False, "Duration must be a number"
    ...
    if 'width' in params and 'height' in params:
        try:
            ...
            if w > 4096 or h > 4096:
                return False, "Dimensions too large (max 4096)"
        except (ValueError, TypeError):
            return False, "Width and height must be integers"

    return True, "Parameters are valid"

def validate_file_size_constraints(size_bytes: int, max_size_mb: float = 100) -> tuple[bool, str]:
    """
...
def validate_text_content(text: str, min_length: int = 1, max_length: int = 10000) -> tuple[bool, str]:
    """
    Validate text content with length and content checks.
    ...
    if not isinstance(text, str):
        return False, "Text content must be a string"
    ...
    for char in problematic_chars:
        if char in text:
            return False, f"Text contains prohibited character: {repr(char)}"

    return True, "Text content is valid"


def enhanced_validation_check(value):
