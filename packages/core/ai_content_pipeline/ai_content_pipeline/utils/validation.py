...
def validate_image_dimensions(width: int, height: int, max_width: int = 2048, max_height: int = 2048) -> tuple[bool, str]:
    ...
    if width <= 0 or height <= 0:
        return False, "Width and height must be positive", "Width and height must be positive"

    if width > max_width or height > max_height:
        return False, f"Dimensions too large (max: {max_width}x{max_height})", f"Dimensions too large (max: {max_width}x{max_height})"
