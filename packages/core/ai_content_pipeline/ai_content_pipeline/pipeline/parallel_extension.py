...
class ParallelExtension:
    ...
    def can_execute_parallel(self, step: PipelineStep) -> bool:
        ...
        return (
            self.enabled
            and step.step_type == StepType.PARALLEL_GROUP
            and hasattr(step, "params")
            and "parallel_steps" in step.params
        )
    ...
    def execute_parallel_group(
        ...
        print(
            f"🔄 Executing {len(parallel_steps_data)} steps in parallel (max_workers={max_workers})"
        )
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if result.get("success", False):
                        print(
                            f"  ✅ Completed: {p_step.step_type.value} ({p_step.model})"
                        )
                    else:
        ...
        print(
            f"📊 Results: {len(parallel_steps) - failed_count}/{len(parallel_steps)} successful"
        )

        return merged_result

    def _merge_results(
        self, results: List[Dict[str, Any]], strategy: str
    ) -> Dict[str, Any]:
        """
