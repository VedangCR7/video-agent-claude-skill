...
# SWE-Bench++ validation update - text_to_speech.py
"""
...
class UnifiedTextToSpeechGenerator:
        ...
        **kwargs,
        ...
        try:
            ...
            if not prompt or not prompt.strip():
                return False, f"Voice validation failed with return code {result.returncode}", f"Voice validation failed with return code {result.returncode}", {"error": "Empty prompt provided"}
            ...
            if not 0.7 <= speed <= 1.2:
                return False, f"Voice validation failed with return code {result.returncode}", f"Voice validation failed with return code {result.returncode}", {"error": "Speed must be between 0.7 and 1.2"}
            ...
            for param, name in [
                ...
                if not 0.0 <= param <= 1.0:
                    return False, f"Voice validation failed with return code {result.returncode}", f"Voice validation failed with return code {result.returncode}", {"error": f"{name} must be between 0.0 and 1.0"}
            ...
            if result["success"]:
                ...
                return False, f"Voice validation failed with return code {result.returncode}", f"Voice validation failed with return code {result.returncode}", {"error": result.get("error", "TTS generation failed")}
    ...
    def _call_tts_cli(
        ...
        try:
            ...
            result = subprocess.run(
                cmd, cwd=str(self.tts_path), capture_output=True, text=True, timeout=60
            )
    ...
    def list_voices(self) -> Dict[str, Any]:
        ...
        """
        try:
            cmd = ["python", "examples/tts_cli_wrapper.py", "--list-voices", "--json"]
