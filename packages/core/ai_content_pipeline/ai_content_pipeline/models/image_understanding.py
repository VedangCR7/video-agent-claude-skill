        ...
        **kwargs,
        ...
        try:
            ...
            if model == "gemini_describe":
                result_text = self.analyzer.describe_image(
                    actual_image_path, detailed=False
                )
            elif model == "gemini_detailed":
                result_text = self.analyzer.describe_image(
                    actual_image_path, detailed=True
                )
            elif model == "gemini_classify":
                result_text = self.analyzer.classify_image(actual_image_path)
            elif model == "gemini_objects":
                result_text = self.analyzer.detect_objects(
                    actual_image_path, detailed=True
                )
                ...
                question = analysis_prompt or kwargs.get(
                    "question", "What do you see in this image?"
                )
                result_text = self.analyzer.answer_image_questions(
                    actual_image_path, [question]
                )
            else:
                return ImageUnderstandingResult(
                    success=False, error=f"Unsupported model: {model}"
                )
