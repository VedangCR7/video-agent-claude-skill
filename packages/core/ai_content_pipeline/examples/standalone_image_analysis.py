...
from ai_content_pipeline.models.image_understanding import (
    UnifiedImageUnderstandingGenerator,
)
...
def example_standalone_image_analysis():
    ...
    if not analyzer.analyzer:
        print(
            "❌ Gemini analyzer not available. Please check GEMINI_API_KEY environment variable."
        )
    ...
    if result.success:
        ...
        print(
            f"   ⏱️ Time: {result.processing_time:.2f}s | 💰 Cost: ${result.cost_estimate:.3f}"
        )
    ...
    if result.success:
        output_preview = (
            str(result.output_text)[:100] + "..."
            if len(str(result.output_text)) > 100
            else str(result.output_text)
        )
...
def example_standalone_prompt_generation():
    ...
    if not generator.analyzer:
        print(
            "❌ OpenRouter analyzer not available. Please check OPENROUTER_API_KEY environment variable."
        )
        print(
            "💡 Set up OpenRouter API key: export OPENROUTER_API_KEY='your_api_key_here'"
        )
        return False
...
def example_local_image_analysis():
    ...
    if not sample_image:
        ...
        print(
            "   cd /home/zdhpe/veo3-video-generation && python ai_content_pipeline/examples/basic_usage.py"
        )
        return True
    ...
    if analyzer.analyzer:
        ...
        if result.success:
            output_preview = (
                str(result.output_text)[:150] + "..."
                if len(str(result.output_text)) > 150
                else str(result.output_text)
            )
...
def example_custom_qa_analysis():
    ...
    for i, question in enumerate(custom_questions, 1):
        ...
        result = analyzer.analyze(
            test_image_url, model="gemini_qa", analysis_prompt=question
        )
...
def main():
    ...
    if successful > 0:
        print("\n💡 Key Takeaways:")
        print(
            "• Image analysis models can be used directly without pipeline validation"
        )
        print(
            "• Both Gemini (image analysis) and OpenRouter (prompt generation) are available"
        )
    ...
    print(
        "• Try: python ai_content_pipeline/examples/test_prompt_generation_standalone.py"
    )
    print(
        "• Try: python -m ai_content_pipeline generate-image --text 'your prompt here'"
    )
