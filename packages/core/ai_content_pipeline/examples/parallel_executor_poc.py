...
def execute_parallel_tts_threads(configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=len(configs)) as executor:
        # Submit all tasks
        future_to_config = {
            executor.submit(execute_single_tts, config): config for config in configs
        }
        ...
        for future in as_completed(future_to_config):
            ...
            try:
                ...
                results.append(
                    {"config": config, "success": False, "result": {"error": str(e)}}
                )
...
def main():
    ...
    print("=" * 50)
    print(
        f"Sequential successful: {sum(1 for r in seq_results if r['success'])}/{len(seq_results)}"
    )
    print(
        f"Parallel successful: {sum(1 for r in par_results if r['success'])}/{len(par_results)}"
    )
