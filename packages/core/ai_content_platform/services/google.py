...
class GoogleVeoStep(BaseStep):
    ...
    def __init__(self, config: StepConfig):
        ...
        self.project_id = (
            self.config.parameters.get("project_id") or self._get_project_id_from_env()
        )
    ...
    async def execute(self, context: Dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            if image_path:
                ...
                result = await self._generate_video_from_image(
                    prompt, image_path, model_name, duration
                )
            else:
                # Text-to-video generation
                result = await self._generate_video_from_text(
                    prompt, model_name, duration
                )
            ...
            if result.startswith("gs://"):
                ...
                output_path = await self.file_manager.download_file(
                    result, output_dir, f"{self.config.name}_{int(time.time())}.mp4"
                )
    ...
    async def _generate_video_from_text(
        self, prompt: str, model_name: str, duration: int
    ) -> str:
        ...
        try:
            ...
            self.logger.info(
                "Simulating Veo video generation (actual API requires allowlist)"
            )
            ...
            raise StepExecutionError(
                f"Failed to generate video from text: {str(e)}"
            ) from e

    async def _generate_video_from_image(
        self, prompt: str, image_path: str, model_name: str, duration: int
    ) -> str:
        ...
        try:
            ...
            if image_path.startswith("http"):
                ...
                local_image = await self.file_manager.download_file(
                    image_path, temp_dir
                )
            else:
            ...
            raise StepExecutionError(
                f"Failed to generate video from image: {str(e)}"
            ) from e
