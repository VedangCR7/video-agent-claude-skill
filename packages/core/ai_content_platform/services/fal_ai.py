...
class BaseFALStep(BaseStep):
    ...
    async def _download_result(self, result_url: str, output_dir: Path) -> Path:
        ...
        try:
            filename = (
                f"{self.config.name}_{int(time.time())}.{self._get_file_extension()}"
            )
            return await self.file_manager.download_file(
                result_url, output_dir, filename
            )
    ...
    async def execute(self, context: Dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            result = await asyncio.to_thread(
                fal_client.subscribe, endpoint, arguments=request_data
            )
...
class FALImageToImageStep(BaseFALStep):
    ...
    async def execute(self, context: Dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            # Submit request
            result = await asyncio.to_thread(
                fal_client.subscribe, "fal-ai/luma-photon-flash", arguments=request_data
            )
...
class FALTextToVideoStep(BaseFALStep):
    ...
    def estimate_cost(self) -> float:
        ...
        cost_map = {"minimax-hailuo-02-pro": 0.08, "google-veo-3": 4.0}
...
class FALVideoGenerationStep(BaseFALStep):
    ...
    async def execute(self, context: Dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            return self._create_result(
                success=True,
                output_path=str(output_path),
                metadata={"model": model, "prompt": prompt, "video_url": video_url},
                execution_time=time.time() - start_time,
            )
    ...
    def estimate_cost(self) -> float:
        ...
        cost_map = {"minimax-hailuo-02": 0.05, "kling-video-2-1": 0.07}
            ...
            if "text" in params:
                ...
                request_data = {
                    ...
                    **{
                        k: v
                        for k, v in params.items()
                        if k not in ["audio_url", "image_url"]
                    },
                }
            else:
                raise StepExecutionError(
                    "Avatar step requires either 'text' or 'audio_url'"
                )
