...
from ai_content_platform.core.exceptions import ConfigurationError, ValidationError
...
@click.option(
    "--parallel/--no-parallel", default=True, help="Enable/disable parallel execution"
)
...
@click.option(
    "--dry-run", is_flag=True, help="Validate and estimate costs without execution"
)
@click.option("--verbose", "-v", is_flag=True, help="Verbose output")
@click.pass_context
async def run(ctx, config_file, parallel, max_workers, output_dir, dry_run, verbose):
    ...
    try:
        ...
        if cost_summary.total_estimated_cost > 1.0:
            if not click.confirm(
                f"Proceed with estimated cost of ${cost_summary.total_estimated_cost:.2f}?"
            ):
        ...
        if parallel:
            executor = ParallelPipelineExecutor(
                config=config, max_workers=max_workers, enable_parallel=True
            )
    ...
    try:
        ...
        if format == "json":
            ...
            output = {
                ...
                "by_step_type": {
                    k.value: v for k, v in cost_summary.by_step_type.items()
                },
                "estimates": [
                    {
...
@click.option(
    "--output", type=click.Path(), default="pipeline.yaml", help="Output file path"
)
def config_create(template, output):
    ...
    try:
        ...
        output_path = config_loader.save_pipeline_config(
            template_config, output, format="yaml"
        )
...
@click.command()
@click.option(
    "--services", is_flag=True, help="Show available services and their status"
)
@click.option("--steps", is_flag=True, help="Show available step types")
@click.option("--models", is_flag=True, help="Show available AI models")
...
def display_cost_summary(cost_summary, detailed=False):
    ...
    # Main cost table
    table = Table(
        title="Cost Estimation", show_header=True, header_style="bold magenta"
    )
...
def display_pipeline_results(result):
    ...
    for step_result in result.step_results:
        status = (
            "[green]✓ Success[/green]" if step_result.success else "[red]✗ Failed[/red]"
        )
        ...
        table.add_row(step_result.step_id, status, time_str, cost_str, output)
...
def check_service_requirements(config):
    ...
    if missing_services:
        console.print(
            f"[yellow]⚠ Missing API keys for: {', '.join(missing_services)}[/yellow]"
        )
