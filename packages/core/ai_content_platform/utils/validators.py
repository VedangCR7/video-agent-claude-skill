...
class ConfigValidator:
    ...
    def _validate_steps(
        self, steps: List[Union[StepConfig, ParallelStepConfig]]
    ) -> None:
    ...
    def _validate_tts_parameters(self, params: Dict[str, Any]) -> None:
        ...
        if "voice_settings" in params and not isinstance(
            params["voice_settings"], dict
        ):
    ...
    def _validate_global_config(self, global_config: Dict[str, Any]) -> None:
        ...
        if "max_cost" in global_config:
            if (
                not isinstance(global_config["max_cost"], (int, float))
                or global_config["max_cost"] < 0
            ):
                raise ValidationError("max_cost must be a non-negative number")

        if "timeout" in global_config:
            if (
                not isinstance(global_config["timeout"], (int, float))
                or global_config["timeout"] <= 0
            ):
