...
class FileManager:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    upload_url, data=data, headers=headers or {}
                ) as response:
                    response.raise_for_status()
    ...
    async def list_files(
        ...
        try:
            ...
            if recursive:
                files = await asyncio.to_thread(lambda: list(directory.rglob(pattern)))
            else:
                files = await asyncio.to_thread(lambda: list(directory.glob(pattern)))
