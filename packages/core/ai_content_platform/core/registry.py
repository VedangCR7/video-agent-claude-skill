...
def register_all_steps():
    ...
    try:
        ...
        from ai_content_platform.services.openrouter import (
            OpenRouterTTSStep,
            OpenRouterChatStep,
        )
        ...
        logger.success(
            f"Registered {len(registered_types)} step types: {[t.value for t in registered_types]}"
        )
...
def register_step_provider_variant(step_type: StepType, provider: str, step_class):
    ...
    try:
        ...
        logger.error(
            f"Failed to register {provider} variant for {step_type.value}: {e}"
        )
...
def register_alternative_providers():
    ...
    try:
        ...
        register_step_provider_variant(
            StepType.TEXT_TO_SPEECH, "openrouter", OpenRouterTTSStep
        )
        register_step_provider_variant(
            StepType.TEXT_GENERATION, "openrouter", OpenRouterChatStep
        )
