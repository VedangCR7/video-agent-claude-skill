...
class ParallelPipelineExecutor(PipelineExecutor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"Parallel execution {'enabled' if enable_parallel else 'disabled'}"
        )
        if enable_parallel:
    ...
    def _analyze_parallel_opportunities(self) -> Dict[str, Any]:
        ...
        if parallel_groups > 0:
            execution_plan["estimated_speedup"] = min(
                2.5, 1.0 + (parallel_groups * 0.3)
            )
    ...
    async def _execute_enhanced_parallel_step(
        self, parallel_config: ParallelStepConfig
    ) -> StepResult:
        ...
        try:
            ...
            merged_result = self._merge_parallel_results(
                results, parallel_config.merge_strategy
            )
    ...
    async def _execute_async_parallel(
        self, step_instances: List[tuple]
    ) -> List[StepResult]:
        ...
        for step, config in step_instances:
            task = asyncio.create_task(self._execute_step_with_context(step, config))
            tasks.append(task)
    ...
    def _execute_step_sync_wrapper(
        self, step: BaseStep, config: StepConfig
    ) -> StepResult:
    ...
    def _calculate_performance_stats(self) -> None:
        ...
        if estimated_sequential_time > 0:
            self.parallel_stats["speedup_factor"] = (
    ...
    async def _setup_pipeline(self) -> None:
        ...
        if max_cost:
            self.cost_calculator.check_cost_limit(
                cost_summary.total_estimated_cost, max_cost
            )
