...
class TestPipelineExecution:
    ...
    def mock_step_execution(self):
        ...
        with (
            patch("ai_content_platform.services.fal_ai.fal_client") as mock_fal,
            patch("ai_content_platform.services.elevenlabs.generate") as mock_eleven,
        ):
            # Mock FAL AI responses
            mock_fal.subscribe = AsyncMock(
                return_value={
            ...
            yield {"fal": mock_fal, "elevenlabs": mock_eleven}

    @pytest.mark.asyncio
    async def test_sequential_pipeline_execution(
        self, sample_pipeline_config, mock_step_execution, mock_env_vars
    ):
    ...
    async def test_parallel_pipeline_execution(
        ...
        executor = ParallelPipelineExecutor(
            config=sample_pipeline_config, enable_parallel=True, max_workers=2
        )
    ...
    async def test_pipeline_with_failed_step(
        self, sample_pipeline_config, mock_env_vars
    ):
    ...
    async def test_pipeline_with_parallel_steps(
        self, temp_dir, mock_step_execution, mock_env_vars
    ):
...
class TestStepFactory:
    ...
    def test_create_fal_ai_steps(self, mock_env_vars):
        ...
        image_config = StepConfig(
            name="test_image",
            step_type=StepType.TEXT_TO_IMAGE,
            parameters={"prompt": "Test image", "model": "flux-1-dev"},
        )
    ...
    def test_create_elevenlabs_step(self, mock_env_vars):
        ...
        tts_config = StepConfig(
            name="test_tts",
            step_type=StepType.TEXT_TO_SPEECH,
            parameters={"text": "Hello world", "voice_id": "test_voice"},
        )
...
class TestConfigurationIntegration:
    ...
    async def test_yaml_config_to_execution(
        self, test_config_yaml, mock_step_execution, mock_env_vars
    ):
    ...
    async def test_json_config_to_execution(
        self, test_config_json, mock_step_execution, mock_env_vars
    ):
