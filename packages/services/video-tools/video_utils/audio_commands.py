...
def cmd_add_audio():
    ...
    if not input_dir.exists():
        print(
            "📁 Input folder not found. Please create an 'input' folder with video and audio files."
        )
        return
...
def cmd_extract_audio():
    ...
    if not input_dir.exists():
        print(
            "📁 Input folder not found. Please create an 'input' folder with video files."
        )
        return
...
def cmd_mix_audio():
    ...
    for video_path in video_files:
        ...
        if add_audio_to_video(
            video_path, mixed_audio_path, output_path, replace_audio=True
        ):
            successful += 1
        else:
            failed += 1
    ...
    try:
        keep_mixed = (
            input(f"\n🗂️  Keep mixed audio file '{mixed_audio_path.name}'? (y/N): ")
            .strip()
            .lower()
        )
        if keep_mixed != "y":
...
def cmd_concat_audio():
    ...
    for video_path in video_files:
        ...
        if add_audio_to_video(
            video_path, concat_audio_path, output_path, replace_audio=True
        ):
            successful += 1
        else:
            failed += 1
    ...
    try:
        keep_concat = (
            input(
                f"\n🗂️  Keep concatenated audio file '{concat_audio_path.name}'? (y/N): "
            )
            .strip()
            .lower()
        )
