...
class WhisperTranscriber:
    ...
    def _transcribe_api(
        self, audio_path: Path, language: Optional[str], response_format: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_size > 25:
                raise ValueError(
                    f"File too large for API: {file_size:.1f}MB (max 25MB)"
                )
    ...
    def _transcribe_local(
        self, audio_path: Path, model_size: str, include_timestamps: bool
    ) -> Dict[str, Any]:
        """Transcribe using local Whisper model."""
        if (
            not self.local_model
            or self.local_model.dims.n_vocab
    ...
    def transcribe_video_audio(
        self, video_path: Path, extract_audio: bool = True, **kwargs
    ) -> Dict[str, Any]:
    ...
    def _extract_audio_from_video(self, video_path: Path) -> Path:
        ...
        try:
            ...
            cmd = [
                ...
                "-vn",  # No video
                "-acodec",
                "pcm_s16le",  # PCM format for compatibility
                "-ar",
...
def check_whisper_requirements(
    check_api: bool = True, check_local: bool = True
