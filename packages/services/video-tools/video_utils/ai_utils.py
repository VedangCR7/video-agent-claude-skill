...
def analyze_media_comprehensively(
    file_path: Path, output_dir: Optional[Path] = None, save_results: bool = True
) -> Dict[str, Any]:
    ...
    if file_ext in video_extensions:
        ...
        try:
            ...
            # Object detection
            obj_result = analyze_image_file(
                file_path, "object_detection", detailed=True
            )
            if obj_result:
...
def print_ai_status():
    ...
    all_available = (
        gemini_available
        and (
            whisper_req.get("api", (False, ""))[0]
            or whisper_req.get("local", (False, ""))[0]
        )
        and whisper_req.get("ffmpeg", (False, ""))[0]
    )
    ...
    if all_available:
        ...
        if (
            not whisper_req.get("api", (False, ""))[0]
            and not whisper_req.get("local", (False, ""))[0]
        ):
            print(
                "   Whisper: Set OPENAI_API_KEY or install: pip install openai-whisper"
            )
