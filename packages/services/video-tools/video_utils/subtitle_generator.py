...
def generate_srt_subtitle_file(
    text: str,
    output_path: Path,
...
def generate_vtt_subtitle_file(
    text: str,
    output_path: Path,
    duration: Optional[float] = None,
    start_time: float = 0.0,
...
def generate_subtitle_for_video(
    ...
    try:
        ...
        if format_type.lower() == "vtt":
            success = generate_vtt_subtitle_file(
                text, subtitle_path, video_duration, 0.0, words_per_second
            )
        else:  # Default to SRT
            success = generate_srt_subtitle_file(
                text, subtitle_path, video_duration, 0.0, words_per_second
            )
...
def add_subtitles_to_video(
    ...
    try:
        cmd = [
            ...
            str(video_path),
            "-vf",
            f"subtitles={subtitle_path}:force_style='FontSize={font_size},PrimaryColour=&H{font_color.replace('#', '')},BackColour=&H{bg_color.replace('#', '')},BorderStyle=3,Outline=1,Shadow=1'",
            "-c:a",
            "copy",
            str(output_path),
            "-y",
...
def add_text_subtitles_to_video(
    video_path: Path,
    text: str,
    output_path: Path,
    font_size: int = 24,
    ...
    try:
        ...
        if not generate_srt_subtitle_file(
            text, subtitle_path, video_duration, 0.0, words_per_second
        ):
            return False

        success = add_subtitles_to_video(
            video_path, subtitle_path, output_path, font_size, font_color, bg_color
        )
