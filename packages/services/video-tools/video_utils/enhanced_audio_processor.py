...
class AudioProcessor:
    ...
    def get_audio_info(self, audio_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            stream_info = (
                result_stream.stdout.strip().split(",")
                if result_stream.stdout.strip()
                else []
            )
            ...
            return {
                ...
                "sample_rate": int(stream_info[1])
                if len(stream_info) > 1 and stream_info[1]
                else None,
                "channels": int(stream_info[2])
                if len(stream_info) > 2 and stream_info[2]
                else None,
                "stream_bit_rate": int(stream_info[3])
                if len(stream_info) > 3 and stream_info[3]
                else None,
            }
    ...
    def mix_files(
        ...
        try:
            ...
            if normalize:
                ...
                filter_complex = (
                    f"{mix_part}amix=inputs={len(audio_files)}:duration=longest[out]"
                )
            ...
            if self.verbose:
                print(
                    f"🎵 Mixing {len(audio_files)} audio files using {method} method..."
                )
            ...
            if result.returncode == 0:
                ...
                if self.verbose:
                    print("❌ Error mixing audio files:")
                    print(result.stderr)
                return False
        ...
        try:
            if crossfade_duration > 0:
                ...
                return self._concatenate_with_crossfade(
                    audio_files, output_path, crossfade_duration
                )
            else:
    ...
    def _concatenate_simple(self, audio_files: List[Path], output_path: Path) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if self.verbose:
                    print("❌ Error concatenating audio files:")
                    print(result.stderr)
                return False
    ...
    def _concatenate_with_crossfade(
        self, audio_files: List[Path], output_path: Path, crossfade_duration: float
    ) -> bool:
        ...
        for i in range(1, len(audio_files)):
            crossfade_filter = (
                f"{current_label}[{i}:a]acrossfade=d={crossfade_duration}[cf{i}]"
            )
        ...
        if self.verbose:
            print(
                f"🎵 Concatenating {len(audio_files)} files with {crossfade_duration}s crossfade..."
            )
        ...
        if result.returncode == 0:
            ...
            if self.verbose:
                print("❌ Error in crossfade concatenation:")
                print(result.stderr)
            return False
    ...
    def trim_audio(
        ...
        try:
            ...
            cmd.extend(["-c:a", "mp3", "-ab", "192k", str(output_path), "-y"])
    ...
    def batch_process(
        self, input_dir: Path, output_dir: Path, operation: str, **kwargs
    ) -> Dict[str, bool]:
