...
def cmd_analyze_images_openrouter():
    ...
    try:
        model_choice = input(
            f"\n🔢 Select model (1-{len(available_models)}, default=1): "
        ).strip()
        if not model_choice:
            model_choice = "1"
        ...
        for image_path in image_files:
            ...
            try:
                ...
                if result_text:
                    ...
                    output_file = (
                        output_dir
                        / f"{image_path.stem}_{analysis_type}_openrouter.json"
                    )
                    if save_analysis_result(result, output_file):
                        ...
                        txt_file = (
                            output_dir
                            / f"{image_path.stem}_{analysis_type}_openrouter.txt"
                        )
    ...
    try:
        ...
        for i, image_path in enumerate(image_files, 1):
            ...
            try:
                ...
                gemini_result = gemini_analyzer.describe_image(
                    image_path, detailed=False
                )
                gemini_time = time.time() - start_time
                gemini_desc = (
                    gemini_result.get("description", "Analysis failed")
                    if gemini_result
            ...
            try:
                start_time = time.time()
                openrouter_desc = openrouter_analyzer.describe_image(
                    image_path, detailed=False
                )
            ...
            comparison_result = {
                ...
                "comparison": {
                    "speed_winner": "gemini"
                    if gemini_time < openrouter_time
                    else "openrouter",
