...
from .subtitle_generator import generate_subtitle_for_video, add_text_subtitles_to_video
...
def cmd_generate_subtitles():
    ...
    try:
        ...
        try:
            ...
            format_choice = input("📄 Format - 1) SRT (default), 2) WebVTT: ").strip()
        ...
        for video_path in video_files:
            ...
            if subtitle_path:
                ...
                print(
                    f"💡 Load this file in your video player alongside {video_path.name}"
                )
            else:
                failed += 1
...
def cmd_burn_subtitles():
    ...
    try:
        ...
        try:
            ...
            font_size = int(input("🔤 Font size (default: 24): ").strip() or "24")
            font_color = input("🎨 Font color (default: white): ").strip() or "white"
        except ValueError:
            print("⚠️  Using default values")
            words_per_second = 2.0
            font_size = 24
        ...
        for video_path in video_files:
            ...
            if add_text_subtitles_to_video(
                video_path,
                subtitle_text,
                output_path,
                font_size,
...
def cmd_generate_subtitles_with_params(
    input_path=None, output_path=None, format_type=None
):
    ...
    if input_path:
        ...
        if input_path.is_file():
            ...
            video_files = (
                [input_path]
                if input_path.suffix.lower()
                in {".mp4", ".avi", ".mov", ".mkv", ".webm", ".flv"}
                else []
            )
            if not video_files:
        ...
        for i, video_path in enumerate(video_files):
            ...
            try:
                ...
                if use_custom_filename and len(video_files) == 1:
                    output_subtitle_path = (
                        output_dir / f"{custom_filename}.{output_format}"
                    )
                else:
                    output_subtitle_path = (
                        output_dir / f"{video_path.stem}.{output_format}"
                    )
