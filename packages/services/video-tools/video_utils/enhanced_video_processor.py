...
class VideoProcessor:
    ...
    def get_video_info(self, video_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            video_info = (
                result_video.stdout.strip().split(",")
                if result_video.stdout.strip()
                else []
            )
            ...
            audio_info = (
                result_audio.stdout.strip().split(",")
            ...
            return {
                ...
                if len(video_info) > 1 and video_info[1]
                else None,
                "height": int(video_info[2])
                if len(video_info) > 2 and video_info[2]
                else None,
                ...
                "sample_rate": int(audio_info[1])
                if len(audio_info) > 1 and audio_info[1]
                else None,
                "audio_channels": int(audio_info[2])
                if len(audio_info) > 2 and audio_info[2]
                else None,
            }
    ...
    def cut_duration(
        self, input_path: Path, output_path: Path, duration: int, start_time: int = 0
    ) -> bool:
    ...
    def cut_timeframe(
        self, input_path: Path, output_path: Path, start_time: str, end_time: str
    ) -> bool:
    ...
    def convert_format(
        self, input_path: Path, output_path: Path, target_codec: str = "libx264"
    ) -> bool:
    ...
    def extract_frames(
        self, input_path: Path, output_dir: Path, frame_rate: str = "1/10"
    ) -> bool:
