...
class MediaProcessingController(BaseController):
    ...
    def cmd_cut_videos(self) -> bool:
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            if info["duration"] is None:
                if self.verbose:
                    print(
                        f"⚠️  Warning: Could not determine duration of {video_path.name}"
                    )
            ...
            output_path = (
                self.output_dir
                / f"{video_path.stem}_cut_{duration}s{video_path.suffix}"
            )
            ...
            # Cut the video
            if self.video_processor.cut_duration(
                video_path, output_path, duration, start_time
            ):
                successful += 1
    ...
    def cmd_add_audio(self) -> bool:
        ...
        if not audio_files:
            if self.verbose:
                ...
                print(
                    "💡 Add some audio files (.mp3, .wav, .aac, etc.) to the input folder"
                )
            return False
        ...
        if not silent_videos:
            if self.verbose:
                ...
                print(
                    "💡 All videos already have audio. Use 'replace-audio' to replace existing audio"
                )
            return False
        ...
        for i, video_path in enumerate(silent_videos, 1):
            ...
            # Create output filename
            output_path = (
                self.output_dir / f"{video_path.stem}_with_audio{video_path.suffix}"
            )
            ...
            # Add audio to video
            if self.audio_processor.add_to_video(
                video_path, selected_audio, output_path, replace_audio=False
            ):
                successful += 1
            else:
                failed += 1
    ...
    def cmd_replace_audio(self) -> bool:
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            output_path = (
                self.output_dir / f"{video_path.stem}_new_audio{video_path.suffix}"
            )
            ...
            if self.audio_processor.add_to_video(
                video_path, selected_audio, output_path, replace_audio=True
            ):
                successful += 1
            else:
                failed += 1
    ...
    def cmd_extract_audio(self) -> bool:
        ...
        for i, video_path in enumerate(videos_with_audio, 1):
            ...
            if self.audio_processor.extract_from_video(
                video_path, output_path, audio_format, quality
            ):
                successful += 1
            else:
                failed += 1
    ...
    def cmd_mix_audio(self) -> bool:
        ...
        method = self.get_user_choice(
            "🎵 Mixing method:", {"1": "Overlay (blend)", "2": "Sum (add)"}, "1"
        )
        method_name = "overlay" if method == "1" else "sum"

        output_path = self.output_dir / "mixed_audio.mp3"

        if self.audio_processor.mix_files(
            selected_files, output_path, normalize, method_name
        ):
            if self.verbose:
    ...
    def cmd_concat_audio(self) -> bool:
        ...
        if self.audio_processor.concatenate_files(
            selected_files, output_path, crossfade_duration
        ):
            if self.verbose:
    ...
    def cmd_resize_videos(self) -> bool:
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            output_path = (
                self.output_dir / f"{video_path.stem}_resized{video_path.suffix}"
            )

            if self.check_skip_existing(output_path):
                continue

            if self.video_processor.resize_video(
                video_path, output_path, width, height, maintain_aspect
            ):
                successful += 1
            else:
                failed += 1
    ...
    def cmd_convert_videos(self) -> bool:
        ...
        codec_choice = self.get_user_choice(
            "🔄 Select target codec:", codec_options, "1"
        )
        codec_map = {"1": "libx264", "2": "libx265", "3": "libvpx-vp9", "4": "copy"}
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            if self.video_processor.convert_format(
                video_path, output_path, target_codec
            ):
                successful += 1
            else:
                failed += 1
    ...
    def cmd_extract_thumbnails(self) -> bool:
        ...
        # Get timestamp
        timestamp = self.get_user_input(
            "⏱️  Timestamp for thumbnail (HH:MM:SS)", "00:00:01"
        )
    ...
    def cmd_batch_process(self) -> bool:
        ...
        operation_choice = self.get_user_choice(
            "⚡ Select batch operation:", batch_options
        )
