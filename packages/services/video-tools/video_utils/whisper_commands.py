...
from .whisper_transcriber import check_whisper_requirements
...
def cmd_whisper_transcribe():
    ...
    if not input_dir.exists():
        ...
        print(
            "💡 Please create the 'input' directory and place your audio/video files there."
        )
        return
    ...
    lang_input = input(
        "🌍 Specify language (e.g., 'en', 'es', 'fr') or press Enter for auto-detect: "
    ).strip()
    if lang_input:
        language = lang_input
    ...
    try:
        ...
        if successful > 0:
            ...
            print(
                f"📄 Files saved to '{output_dir}' with '_whisper_transcription' suffix"
            )
...
def cmd_whisper_compare():
    ...
    for i, file_path in enumerate(all_files, 1):
        ...
        try:
            ...
            whisper_result = transcribe_with_whisper(
                file_path, use_local=use_local, include_timestamps=True
            )
            ...
            if file_path.suffix.lower() in {
                ...
                gemini_result = analyzer.transcribe_video(
                    file_path, include_timestamps=True
                )
            else:
                # Audio file - use Gemini audio transcription
                analyzer = GeminiVideoAnalyzer()
                gemini_result = analyzer.transcribe_audio(
                    file_path, include_timestamps=True
                )
            ...
            with open(txt_file, "w", encoding="utf-8") as f:
                ...
                f.write("-" * 30 + "\n")
                f.write(
                    f"Whisper: {comparison['comparison_stats']['whisper_words']} words\n"
                )
                f.write(
                    f"Gemini: {comparison['comparison_stats']['gemini_words']} words\n"
                )
    ...
    if successful_comparisons > 0:
        ...
        print(
            f"📄 Check '{output_dir}' for '_comparison.txt' files with readable results"
        )
        print(
            f"📊 Check '{output_dir}' for '_comparison.json' files with detailed data"
        )
...
def cmd_whisper_batch():
    ...
    if not any(ready for ready, _ in whisper_status.values()):
        ...
        print(
            "💡 Install: pip install openai (API) or pip install openai-whisper (local)"
        )
        return
    ...
    if use_local:
        model_choice = (
            input("🎯 Model (tiny/base/small/medium/large/turbo): ").strip().lower()
        )
        if model_choice in ["tiny", "base", "small", "medium", "large", "turbo"]:
            model_size = model_choice
