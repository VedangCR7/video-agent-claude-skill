...
class ImageModifyVerifySystem:
    ...
    def understand_image(
        self, image_path: Path, analysis_types: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for analysis_type in analysis_types:
            ...
            try:
                if analysis_type == "description":
                    result = self.gemini_analyzer.describe_image(
                        image_path, detailed=True
                    )
                    results["analysis"]["description"] = result["description"]

                elif analysis_type == "objects":
                    result = self.gemini_analyzer.detect_objects(
                        image_path, detailed=True
                    )
                    results["analysis"]["objects"] = result["objects"]

                elif analysis_type == "composition":
                    result = self.gemini_analyzer.analyze_composition(
                        image_path, detailed=True
                    )
    ...
    def suggest_modifications(
        ...
        if any(
            word in description_lower for word in ["dark", "dim", "shadow", "low light"]
        ):
            suggestions.append(
                {
                    "type": "lighting",
        ...
        if any(
            word in description_lower for word in ["dull", "muted", "faded", "pale"]
        ):
            suggestions.append(
                {
        ...
        if any(
            word in description_lower for word in ["landscape", "outdoor", "nature"]
        ):
            suggestions.append(
                {
        ...
        if any(
            ...
            for word in ["blurry", "noisy", "grainy", "low quality"]
        ):
            suggestions.append(
                {
    ...
    def modify_image(
        self, image_path: Path, modification: Dict[str, Any]
    ) -> Optional[Path]:
        """
        ...
        try:
            ...
            print("   📤 Uploading image for modification...")
            ...
            if result and "image_url" in result:
                ...
                if response.status_code == 200:
                    ...
                    modified_path = (
                        self.output_dir / f"{image_path.stem}_modified_{timestamp}.png"
                    )
    ...
    def verify_modification(
        self, original_path: Path, modified_path: Path
    ) -> Dict[str, Any]:
        """
        ...
        # Analyze both images
        original_analysis = self.understand_image(
            original_path, ["description", "objects"]
        )
        modified_analysis = self.understand_image(
            modified_path, ["description", "objects"]
        )
...
Provide a detailed comparison."""
            ...
            verification["comparison"]["summary"] = (
                "Modification completed - manual review recommended"
            )
    ...
    def complete_workflow(
        ...
        try:
            ...
            understanding_file = (
                self.output_dir / f"{image_path.stem}_understanding.json"
            )
            ...
            workflow_file = (
                self.output_dir / f"{image_path.stem}_workflow_complete.json"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Image Understanding + Modification + Verification"
    )
    parser.add_argument("image", help="Path to image file")
    parser.add_argument("--prompt", help="Custom modification prompt")
    parser.add_argument(
        "--model",
        choices=["photon", "photon_base", "kontext"],
        default="photon",
        help="Modification model",
    )
