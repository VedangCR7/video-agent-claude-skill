...
class OpenRouterTTSPipeline:
    ...
    def calculate_length_requirements(
        self, target_minutes: float, num_people: int = 1
    ) -> LengthCalculation:
        """
    ...
    def generate_content_with_llm(
        ...
        length_calc = self.calculate_length_requirements(
            pipeline_input.length_minutes, pipeline_input.num_people
        )
    ...
    def convert_to_speech(
        ...
        try:
            if pipeline_input.num_people == 1:
                ...
                # Multi-speaker
                script = self._parse_dialogue_content(
                    generated_content.content, pipeline_input
                )
                return self.tts_controller.multi_voice_generation(
                    script=script, output_file=output_file
                )
    ...
    def run_complete_pipeline(
        ...
        try:
            ...
            speech_success = self.convert_to_speech(
                generated_content, pipeline_input, output_file
            )
    ...
    def _build_content_prompt(
        self, pipeline_input: PipelineInput, length_calc: LengthCalculation
    ) -> str:
...
Please generate natural, engaging content that fits these requirements."""
    ...
    def _parse_dialogue_content(
        self, content: str, pipeline_input: PipelineInput
    ) -> List[Dict[str, str]]:
        ...
        voice_config = get_voice_style_preset(pipeline_input.voice_style.value)
        voices = (
            voice_config.get("primary_voices", ["rachel", "drew"])
            if voice_config
            else ["rachel", "drew"]
        )
            ...
            if line and ":" in line:
                ...
                if len(parts) == 2:
                    ...
                    if text:
                        ...
                        script.append({"speaker": voice, "text": text})
                        current_speaker += 1
