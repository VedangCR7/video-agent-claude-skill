...
Examples:
  ...
  %(prog)s "Sample text" bella --json
    ...
    parser.add_argument(
        "voice", nargs="?", default="rachel", help="Voice name (default: rachel)"
    )
    parser.add_argument("output", nargs="?", help="Output file path (optional)")

    # Optional parameters
    parser.add_argument(
        "--speed", type=float, default=1.0, help="Speech speed (0.7-1.2)"
    )
    parser.add_argument(
        "--stability", type=float, default=0.5, help="Voice stability (0.0-1.0)"
    )
    parser.add_argument(
        "--similarity-boost", type=float, default=0.8, help="Similarity boost (0.0-1.0)"
    )
    parser.add_argument(
        "--style", type=float, default=0.2, help="Style exaggeration (0.0-1.0)"
    )
    ...
    try:
        ...
        if args.validate_voice:
            ...
            if args.json:
                ...
                print(
                    f"Voice '{args.validate_voice}' is {'valid' if is_valid else 'invalid'}"
                )
