...
class TTSBasicUsage:
    ...
    def _voice_settings_example(self):
        ...
        text = (
            "This demonstrates custom voice settings for different emotional effects."
        )
    ...
    def _all_examples(self):
        ...
        print("=" * 50)
        print(
            "✅ Examples completed! Check the output/ directory for generated audio files."
        )
...
Examples:
  ...
  %(prog)s --list-voices
    ...
    action_group.add_argument("--text", "-t", help="Text to convert to speech")
    ...
    action_group.add_argument(
        "--list-voices", "-l", action="store_true", help="List available voices"
    )

    # Voice and output options
    parser.add_argument(
        "--voice", "-v", default="rachel", help="Voice name (default: rachel)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--api-key", help="ElevenLabs API key (default: from environment)"
    )
