...
class ElevenLabsTTSController:
    ...
    def text_to_speech(
        ...
        headers = self.audio_processor.build_headers_for_format(
            self.headers, audio_format
        )
        ...
        try:
            ...
            # Process response
            if stream:
                return self.audio_processor.process_streaming_response(
                    response, output_file
                )
            else:
                return self.audio_processor.process_regular_response(
                    response, output_file
                )
    ...
    def multi_voice_generation(
        ...
        try:
            ...
            if temp_files and output_file:
                success = self.audio_processor.combine_audio_files(
    ...
    def clone_voice_from_file(
        self, audio_file: str, voice_name: str, description: str = "Custom cloned voice"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            with open(audio_file, "rb") as f:
                ...
                response = make_request_with_retry(
                    url=url, headers=self.headers, data=data, files=files, method="POST"
                )
    ...
    def search_voices(
        self, query: str, category: Optional[str] = None, gender: Optional[str] = None
    ) -> List[VoiceInfo]:
        """Search for voices"""
