...
class VoiceManager:
        ...
        if self._voices_cache is None or refresh_cache:
            try:
                ...
                if response and response.status_code == 200:
                    ...
                    print(
                        "Failed to retrieve voices from API, using popular voices as fallback"
                    )
    ...
    def print_voices(self, category: Optional[str] = None, limit: int = 20):
        ...
        for i, voice in enumerate(voices[:limit]):
            print(
                f"{i + 1:2d}. {voice.name:15} | {voice.voice_id:25} | {voice.category:10} | {voice.gender:8}"
            )
    ...
    def get_random_voice(
        self, category: Optional[str] = None, gender: Optional[str] = None
    ) -> Optional[VoiceInfo]:
        """
