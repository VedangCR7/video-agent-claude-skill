...
def get_voice_choice(voices: list) -> str:
    ...
    while True:
        try:
            choice = input(
                f"\nSelect voice (1-{len(voices)}, name, or Enter for Bill): "
            ).strip()
...
def get_image_input() -> str:
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                img_choice = (
                    int(input(f"Select image (1-{min(len(sample_images), 10)}): ")) - 1
                )
...
def get_audio_input() -> str:
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                audio_choice = (
                    int(input(f"Select audio (1-{min(len(sample_audio), 10)}): ")) - 1
                )
...
def main():
    ...
    try:
        ...
        if mode_choice == "1":
            ...
            print(
                f"✅ Text entered: {text_input[:50]}{'...' if len(text_input) > 50 else ''}"
            )
        ...
        while True:
            try:
                frames_input = input(
                    f"Number of frames (81-129, default {default_frames}): "
                ).strip()
                if not frames_input:
        ...
        # Turbo mode
        turbo_input = (
            input("Use turbo mode for faster generation? (Y/n): ").strip().lower()
        )
        turbo = turbo_input != "n"

        # Custom prompt
        custom_prompt = input(
            "Custom prompt (optional, press Enter for official example): "
        ).strip()
        ...
        mode_names = {
            "1": "Text-to-Speech",
            "2": "Audio-to-Avatar",
            "3": "Multi-Audio Conversation",
        }
        print(f"   - Mode: {mode_names[mode_choice]}")
        print(
            f"   - Image: {os.path.basename(image_input) if os.path.isfile(image_input) else image_input}"
        )
        ...
        if mode_choice == "1":
            ...
            print(
                f"   - Audio: {os.path.basename(audio_input) if os.path.isfile(audio_input) else audio_input}"
            )
        else:
            print(
                f"   - First audio: {os.path.basename(first_audio) if os.path.isfile(first_audio) else first_audio}"
            )
            print(
                f"   - Second audio: {os.path.basename(second_audio) if os.path.isfile(second_audio) else second_audio}"
            )
