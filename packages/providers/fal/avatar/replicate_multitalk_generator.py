...
class ReplicateMultiTalkGenerator:
    ...
    def __init__(self, api_token: Optional[str] = None):
        ...
        if not self.api_token:
            raise ValueError(
                "REPLICATE_API_TOKEN environment variable not set or api_token not provided"
            )
        ...
        print("🗣️ Supports: Multi-person conversational video generation")
    ...
    def generate_conversation_video(
        ...
        """
        try:
            print("🎬 Starting MultiTalk conversational video generation...")
            ...
            if os.path.isfile(first_audio_url):
                ...
                with open(first_audio_url, "rb") as f:
                    first_audio_url = f
                print("✅ First audio prepared for upload")
            ...
            if second_audio_url and os.path.isfile(second_audio_url):
                ...
                with open(second_audio_url, "rb") as f:
                    second_audio_url = f
                print("✅ Second audio prepared for upload")

            # Validate parameters
            if not (25 <= num_frames <= 201):
                raise ValueError(
                    f"num_frames must be between 25 and 201, got {num_frames}"
                )

            if not (2 <= sampling_steps <= 100):
                raise ValueError(
                    f"sampling_steps must be between 2 and 100, got {sampling_steps}"
                )
            ...
            output = replicate.run(self.model_version, input=input_args)
            ...
            if output:
                ...
                print("✅ MultiTalk conversation video generated successfully!")
                ...
                raise Exception("No output received from MultiTalk model")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if model:
                ...
                print(
                    f"🏷️ Latest version: {model.latest_version.id if model.latest_version else 'N/A'}"
                )
                return True
            else:
...
def main():
    ...
    try:
        ...
        print("\n📋 Usage Examples:")
        print(
            "   • Single person: generator.generate_single_person_video(image, audio)"
        )
        print(
            "   • Conversation: generator.generate_conversation_video(image, audio1, audio2)"
        )
        print("   • Example: generator.generate_official_example()")

        print("\n⚠️ Note: MultiTalk requires actual audio files and images.")
        print("   The official_example() method uses placeholders for demonstration.")
