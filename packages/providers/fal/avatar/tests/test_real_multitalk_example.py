...
def test_real_multitalk_example(dry_run=False):
    ...
    try:
        ...
        prompt = (
            "In a casual, intimate setting, a man and a woman are engaged in a heartfelt conversation "
...
def main():
    ...
    print("   • Assets: Real image and audio from Replicate delivery")
    print("   • Scene: Car conversation between man and woman")
    ...
    if success:
        ...
        if not args.dry_run:
            print("📁 Check output/real_multitalk_example.mp4 for the generated video")
    else:
        print("\n❌ Test failed")

    print("\n💡 Usage tips:")
    print("   • Use --dry-run for cost-free validation")
    print("   • Ensure REPLICATE_API_TOKEN is set in .env")
    print("   • Check your Replicate account for usage/billing")
