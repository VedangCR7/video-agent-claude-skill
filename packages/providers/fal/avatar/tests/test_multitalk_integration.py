...
def test_multitalk_generator():
    ...
    try:
        ...
        for expected_feature in expected_features:
            found = any(
                expected_feature.lower() in feature.lower() for feature in features
            )
        ...
        for required_input in required_inputs:
            assert required_input in input_formats, (
                f"❌ Missing input format: {required_input}"
            )
...
def test_multitalk_integration():
    ...
    try:
        ...
        assert hasattr(fal_gen, "generate_avatar_video"), (
            "❌ FAL generator missing generate_avatar_video"
        )
        assert hasattr(replicate_gen, "generate_conversation_video"), (
            "❌ MultiTalk generator missing generate_conversation_video"
        )
        print("✅ Both generators have video generation methods")

        # Test that both have test methods
        assert hasattr(fal_gen, "test_connection"), (
            "❌ FAL generator missing test_connection"
        )
        assert hasattr(replicate_gen, "test_connection"), (
            "❌ MultiTalk generator missing test_connection"
        )
...
if __name__ == "__main__":
    ...
    if passed == total:
        ...
        print(
            "⚠️  Note: Actual generation requires valid Replicate API token and audio files"
        )
        sys.exit(0)
    else:
