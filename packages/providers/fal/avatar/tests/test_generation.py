...
def get_user_confirmation(
    test_name: str, cost_estimate: str, details: str = ""
) -> bool:
...
def get_test_image() -> str:
    ...
    print("📷 Using official FAL AI example image")
...
def test_basic_generation(generator: FALAvatarGenerator, args) -> bool:
    ...
    try:
        ...
        print("🚀 Starting basic avatar generation...")
        ...
        if result and "video" in result:
            print("✅ Basic avatar generation successful!")
            print(f"📁 Saved to: {output_path}")
            print(f"⏱️ Generation time: {result.get('generation_time', 0):.2f} seconds")

            video_info = result["video"]
            print(
                f"📊 File size: {video_info.get('file_size', 0) / (1024 * 1024):.2f} MB"
            )
            return True
        else:
            print("❌ Basic avatar generation failed")
            return False
...
def test_voice_comparison(generator: FALAvatarGenerator, args) -> bool:
    ...
    if len(available_test_voices) < 2:
        available_test_voices = all_voices[
            :3
        ]  # Use first 3 if test voices not available
    ...
    try:
        ...
        for i, voice in enumerate(available_test_voices):
            ...
            if result and "video" in result:
                results.append({"voice": voice, "path": output_path, "result": result})
        ...
        if results:
            ...
            for r in results:
                ...
                print(
                    f"   🎤 {r['voice']}: {r['path']} ({size_mb:.2f} MB, {gen_time:.1f}s)"
                )

            return len(results) > 0
        else:
            print("❌ All voice generations failed")
            return False
    ...
    try:
        ...
        if results:
            ...
            for r in results:
                ...
                gen_time = r["result"].get("generation_time", 0)
                print(
                    f"   🎬 {r['scenario']}: {r['path']} ({size_mb:.2f} MB, {gen_time:.1f}s)"
                )

            return len(results) > 0
        else:
            print("❌ All scenario generations failed")
            return False
...
def test_audio_generation(generator: FALAvatarGenerator, args) -> bool:
    ...
    try:
        ...
        print("🚀 Starting audio-to-avatar generation...")
        print(f"   Image: {image_url}")
        ...
        if result and "video" in result:
            print("✅ Audio-to-avatar generation successful!")
            print(f"📁 Saved to: {output_path}")
            print(f"⏱️ Generation time: {result.get('generation_time', 0):.2f} seconds")
            ...
            print("❌ Audio-to-avatar generation failed")
            return False
...
def test_multi_audio_generation(generator: FALAvatarGenerator, args) -> bool:
    ...
    if not get_user_confirmation(
        "Multi-Audio Conversation Test", estimated_cost, details
    ):
    ...
    try:
        ...
        print("🚀 Starting multi-audio conversation generation...")
        print(f"   Image: {image_url}")
        ...
        if result and "video" in result:
            print("✅ Multi-audio conversation generation successful!")
            print(f"📁 Saved to: {output_path}")
            print(f"⏱️ Generation time: {result.get('generation_time', 0):.2f} seconds")
            ...
            print("❌ Multi-audio conversation generation failed")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="FAL AI Avatar Generation Tests (PAID)"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Quick test with minimal frames (81)"
    )
    parser.add_argument(
        "--standard",
        action="store_true",
        help="Standard test with default frames (136)",
    )
    parser.add_argument(
        "--audio", action="store_true", help="Test audio-to-avatar generation"
    )
    parser.add_argument(
        "--multi", action="store_true", help="Test multi-audio conversation generation"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Compare multiple voices (costs more)"
    )
    parser.add_argument(
        "--scenarios", action="store_true", help="Test custom scenarios"
    )
    parser.add_argument(
        "--voice",
        default="Bill",
        help="Voice to use for testing (default: Bill - from official example)",
    )
    ...
    print("💰 Each video costs approximately $0.02-0.05")
