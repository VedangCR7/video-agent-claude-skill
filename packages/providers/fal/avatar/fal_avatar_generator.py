...
class FALAvatarGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "FAL_KEY environment variable not set or api_key not provided"
            )
        ...
        print("✅ FAL Avatar Generator initialized")
    ...
    def generate_avatar_video(
        ...
        try:
            ...
            if not (81 <= num_frames <= 129):
                raise ValueError(
                    f"num_frames must be between 81 and 129, got {num_frames}"
                )
    ...
    def generate_avatar_from_audio(
        ...
        """
        try:
            print("🎬 Starting avatar video generation from audio...")
            print(f"🖼️ Image: {image_url}")
    ...
    def generate_multi_avatar_conversation(
        ...
        """
        try:
            print("🎬 Starting multi-person avatar conversation generation...")
            print(f"🖼️ Image: {image_url}")
            ...
            if result and "video" in result:
                ...
                print("✅ Multi-avatar conversation generated successfully!")
                print(f"⏱️ Generation time: {generation_time:.2f} seconds")
                print(f"📊 File size: {file_size / (1024 * 1024):.2f} MB")
                print(f"🔗 Video URL: {video_url}")
...
def main():
    ...
    try:
        ...
        print("   - Image: Official FAL AI example image")
        print("   - Text: 'Spend more time with people who make you feel alive...'")
        print("   - Voice: Bill (official example)")
        print("   - Prompt: Podcast-style generation")
        print("   - Use: generator.generate_official_example()")
        print("   - Source: https://fal.ai/models/fal-ai/ai-avatar/single-text/api")
