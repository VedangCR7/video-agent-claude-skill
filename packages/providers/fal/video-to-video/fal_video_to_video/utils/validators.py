...
from ..config.constants import SUPPORTED_MODELS, MAX_VIDEO_SIZE_MB
...
def validate_video_path(video_path: str) -> str:
    ...
    if ext not in valid_extensions:
        raise ValueError(
            f"Unsupported video format: {ext}. Supported: {valid_extensions}"
        )
    ...
    if file_size_mb > MAX_VIDEO_SIZE_MB:
        raise ValueError(
            f"Video file too large: {file_size_mb:.1f}MB. Maximum: {MAX_VIDEO_SIZE_MB}MB"
        )
