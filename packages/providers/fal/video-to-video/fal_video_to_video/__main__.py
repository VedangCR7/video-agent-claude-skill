    ...
    try:
        ...
        if args.video_url:
            result = generator.add_audio_to_video(
                video_url=args.video_url, model=args.model, **kwargs
            )
        else:
            result = generator.add_audio_to_local_video(
                video_path=args.video_path, model=args.model, **kwargs
            )
...
def upscale_video(args):
    ...
    try:
        ...
        # Execute based on input type
        if args.video_url:
            result = generator.upscale_video(video_url=args.video_url, **kwargs)
        else:
            result = generator.upscale_local_video(video_path=args.video_path, **kwargs)
...
Examples:
  ...
  python -m fal_video_to_video batch -f batch.json
    ...
    audio_parser = subparsers.add_parser(
        "add-audio", help="Add audio to a single video"
    )
    ...
    audio_parser.add_argument(
        ...
        default="thinksound",
        help="Model to use (default: thinksound)",
    )
    audio_parser.add_argument(
        "-p", "--prompt", help="Text prompt to guide audio generation"
    )
    audio_parser.add_argument(
    ...
    upscale_input_group.add_argument(
        "-i", "--video-path", help="Path to local video file"
    )
    ...
    upscale_parser.add_argument(
        "--target-fps", type=int, help="Target FPS for frame interpolation (1-120)"
    )
    ...
    batch_parser.add_argument(
        ...
        choices=SUPPORTED_MODELS,
        default="thinksound",
        help="Default model (can be overridden per video)",
    )
