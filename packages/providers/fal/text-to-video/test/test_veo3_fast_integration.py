...
def test_veo3_fast_integration():
    ...
    try:
        ...
        assert hasattr(TextToVideoModel, "GOOGLE_VEO3_FAST"), (
            "❌ GOOGLE_VEO3_FAST model not found"
        )
        assert TextToVideoModel.GOOGLE_VEO3_FAST.value == "fal-ai/veo3/fast", (
            "❌ Incorrect endpoint"
        )
        ...
        fake_api_key = (
            "00000000-0000-0000-0000-000000000000:fakehashfortest123456789abcdef"
        )
        ...
        for key, expected_value in expected_config.items():
            assert model_info[key] == expected_value, (
                f"❌ {key}: expected {expected_value}, got {model_info[key]}"
            )
        ...
        cost_5s_no_audio = generator.calculate_cost(
            TextToVideoModel.GOOGLE_VEO3_FAST, "5s", False
        )
        cost_8s_with_audio = generator.calculate_cost(
            TextToVideoModel.GOOGLE_VEO3_FAST, "8s", True
        )

        assert cost_5s_no_audio == 1.25, f"❌ Expected $1.25, got ${cost_5s_no_audio}"
        assert cost_8s_with_audio == 3.20, (
            f"❌ Expected $3.20, got ${cost_8s_with_audio}"
        )
        ...
        assert "GOOGLE_VEO3_FAST" in all_models["available_models"], (
            "❌ Veo3 Fast not in model list"
        )
        ...
        assert all("Veo 3 Fast" in estimate for estimate in estimates), (
            "❌ Cost estimates don't mention Veo3 Fast"
        )
...
def test_ai_pipeline_integration():
    ...
    try:
        ...
        assert "veo3_fast" in SUPPORTED_MODELS["image_to_video"], (
            "❌ veo3_fast not in SUPPORTED_MODELS"
        )
        print("✅ Model in SUPPORTED_MODELS")

        # Test cost estimate exists
        assert "veo3_fast" in COST_ESTIMATES["image_to_video"], (
            "❌ veo3_fast cost not defined"
        )
        assert COST_ESTIMATES["image_to_video"]["veo3_fast"] == 2.00, (
            "❌ Incorrect cost estimate"
        )
        print("✅ Cost estimate configured")

        # Test recommendation exists
        assert "balanced" in MODEL_RECOMMENDATIONS["image_to_video"], (
            "❌ balanced recommendation missing"
        )
        assert MODEL_RECOMMENDATIONS["image_to_video"]["balanced"] == "veo3_fast", (
            "❌ veo3_fast not recommended for balanced"
        )
