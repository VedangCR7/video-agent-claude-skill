...
def test_environment_variables():
    ...
    if fal_key:
        ...
        print(
            "❌ FAL_KEY found but format appears invalid (should be uuid:hash format)"
        )
        return False
    else:
...
def test_module_import():
    ...
    try:
        from fal_text_to_video_generator import (
        ...
        print(
            f"✅ Model info retrieved: {len(all_models_info['available_models'])} models available"
        )
        ...
        print(
            f"✅ Cost calculation working: MiniMax ${minimax_cost:.2f}, Veo3 ${veo3_cost:.2f}"
        )
...
def main():
    ...
    if passed == total:
        ...
        print("💡 Next steps:")
        print(
            "   1. Run test_generation.py to test actual video generation (costs ~$0.08)"
        )
