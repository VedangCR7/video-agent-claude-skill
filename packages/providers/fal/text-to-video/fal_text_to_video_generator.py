...
class FALTextToVideoGenerator:
    ...
    def generate_video(
        ...
        try:
            ...
            if model == TextToVideoModel.MINIMAX_HAILUO:
                arguments = {"prompt": prompt, "prompt_optimizer": prompt_optimizer}
            ...
            # Process result
            video_url = (
                result.get("video", {}).get("url") if "video" in result else None
            )
            if not video_url:
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if self.verbose and total_size > 0:
                            ...
                            print(
                                f"\r⬇️ Downloading: {progress:.1f}%", end="", flush=True
                            )
    ...
    def get_model_info(
        self, model: Optional[TextToVideoModel] = None
    ) -> Dict[str, Any]:
        ...
        if model is None:
            ...
            return {
                "available_models": {
                    model.name: {"endpoint": model.value, **config}
            ...
            config = self.MODEL_CONFIGS[model]
            return {"model_name": config["name"], "endpoint": model.value, **config}
    ...
    def print_model_comparison(self):
        ...
        for model, config in self.MODEL_CONFIGS.items():
            ...
            if "cost_per_video" in config:
                ...
                print(
                    f"💰 Cost: ${config['cost_per_second_no_audio']:.2f}/s (no audio), ${config['cost_per_second_with_audio']:.2f}/s (with audio)"
                )
...
def main():
    ...
    for duration in ["5s", "6s", "7s", "8s"]:
        print(
            f"  • {generator.get_cost_estimate(TextToVideoModel.GOOGLE_VEO3, duration, True)}"
        )
        print(
            f"  • {generator.get_cost_estimate(TextToVideoModel.GOOGLE_VEO3, duration, False)}"
        )
        print(
            f"  • {generator.get_cost_estimate(TextToVideoModel.GOOGLE_VEO3_FAST, duration, True)}"
        )
        print(
            f"  • {generator.get_cost_estimate(TextToVideoModel.GOOGLE_VEO3_FAST, duration, False)}"
        )
