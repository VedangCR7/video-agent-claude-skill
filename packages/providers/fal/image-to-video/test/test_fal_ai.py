...
def show_cost_warning():
    ...
    print(
        "   --hailuo      : Test fal-ai/minimax/hailuo-02/standard/image-to-video (~$0.02-0.05)"
    )
    print(
        "   --kling       : Test fal-ai/kling-video/v2.1/standard/image-to-video (~$0.02-0.05)"
    )
...
def test_environment():
    ...
    if fal_key.startswith("fal-"):
        ...
        print(
            "⚠️  FAL_KEY found but may not be in correct format (should start with 'fal-')"
        )
    ...
    try:
        ...
        if result and "video" in result:
            ...
            if video_url and video_url != "No URL found":
                ...
                local_path = generator.download_video(
                    video_url, "output", f"test_{model}_video.mp4"
                )
                if local_path:
...
def main():
    ...
    if len(sys.argv) > 1 and not any(
        [quick_test, full_test, api_only, hailuo_test, kling_test, compare_test]
    ):
        show_cost_warning()
    ...
    if hailuo_test and not compare_test:
        ...
        video_ok = test_video_generation(
            generator, quick_test=quick_test, model="hailuo"
        )
