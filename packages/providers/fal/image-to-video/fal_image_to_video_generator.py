...
class FALImageToVideoGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            ...
            if os.environ.get("CI") or os.environ.get("GITHUB_ACTIONS"):
                ...
                raise ValueError(
                    "FAL API key is required. Set FAL_KEY environment variable or pass api_key parameter."
                )
        else:
        ...
        try:
            ...
            if result and "video" in result:
                ...
                local_path = self.download_video(
                    video_url, output_folder, custom_filename
                )
                if local_path:
    ...
    def generate_video_with_kling(
        ...
        """
        try:
            print("Starting video generation with Kling Video 2.1...")
            print(f"Prompt: '{prompt}'")
            print(f"Image URL: {image_url}")
            print(f"Duration: {duration} seconds")
            ...
            if use_async:
                # Async processing
                handler = fal_client.submit(self.kling_endpoint, arguments=arguments)
                ...
                while True:
                    status = fal_client.status(
                        self.kling_endpoint, request_id, with_logs=True
                    )
                    print(f"Status: {status.status}")
    ...
    def download_video(
        self, video_url: str, output_folder: str, filename: str
    ) -> Optional[str]:
        """
...
Supported Models:
  ...
  - kling: Kling Video 2.1 - High-quality, 5-10s, with CFG scale and negative prompts
    ...
    input_group.add_argument("-i", "--image", type=str, help="Path to local image file")
    input_group.add_argument("--url", type=str, help="URL of image to use")
    ...
    # Processing options
    parser.add_argument(
        "--async", action="store_true", help="Use async processing with polling"
    )
    ...
    # API key
    parser.add_argument(
        "--api-key", type=str, help="FAL API key (if not set in environment)"
    )
...
def validate_arguments(args):
    ...
    if not args.api_key and not os.getenv("FAL_KEY"):
        errors.append(
            "FAL API key required. Set FAL_KEY environment variable or use --api-key"
        )
    ...
    try:
        ...
        if args.image:
            ...
            model_endpoint = (
                ...
                if args.model == "kling"
                else "fal-ai/minimax/hailuo-02/standard/image-to-video"
            )
