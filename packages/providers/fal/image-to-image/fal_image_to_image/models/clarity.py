...
class ClarityModel(BaseModel):
    ...
    def validate_parameters(self, **kwargs) -> Dict[str, Any]:
        ...
        if not isinstance(enable_enhancement, bool):
            raise ValueError(
                f"enable_enhancement must be a boolean, got {type(enable_enhancement).__name__}"
            )

        # Build parameters
        params = {"scale": scale, "enable_enhancement": enable_enhancement}
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {**MODEL_INFO["clarity"], "endpoint": self.endpoint}
