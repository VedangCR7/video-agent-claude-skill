...
def print_models():
    ...
    for model_key, info in MODEL_INFO.items():
        ...
        if model_key == "clarity":
            ...
            print(
                f"   Aspect Ratios: {', '.join(info.get('supported_aspect_ratios', [])[:5])}..."
            )
...
def modify_image(args):
    ...
    try:
        ...
        if args.model == "clarity":
            ...
            if args.image_url:
                result = generator.upscale_image(image_url=args.image_url, **kwargs)
            else:
...
Examples:
  ...
  python -m fal_image_to_image batch -f batch.json -m photon
    ...
    modify_parser.add_argument(
        ...
        default="photon",
        help="Model to use (default: photon)",
    )
    modify_parser.add_argument(
        "-p", "--prompt", help="Modification prompt (not needed for clarity)"
    )

    # Model-specific options
    modify_parser.add_argument(
        "--strength", type=float, help="Strength for Photon models (0.0-1.0)"
    )
    modify_parser.add_argument("--aspect-ratio", help="Aspect ratio for Photon models")
    modify_parser.add_argument(
        "--num-inference-steps", type=int, help="Steps for Kontext"
    )
    ...
    modify_parser.add_argument(
        "--scale", type=float, help="Scale factor for Clarity (1-4)"
    )
    modify_parser.add_argument(
        "--no-enhancement", action="store_true", help="Disable enhancement for Clarity"
    )

    # Reframing options for Photon models
    modify_parser.add_argument(
        "--x-start", type=int, help="Start X coordinate for reframing"
    )
    modify_parser.add_argument(
        "--y-start", type=int, help="Start Y coordinate for reframing"
    )
    modify_parser.add_argument(
        "--x-end", type=int, help="End X coordinate for reframing"
    )
    modify_parser.add_argument(
        "--y-end", type=int, help="End Y coordinate for reframing"
    )
    modify_parser.add_argument(
        "--grid-position-x", type=int, help="X position of grid for reframing"
    )
    modify_parser.add_argument(
        "--grid-position-y", type=int, help="Y position of grid for reframing"
    )
    modify_parser.add_argument(
        "--auto-center",
        action="store_true",
    ...
    modify_parser.add_argument(
        "--input-width", type=int, help="Input image width (for auto-center)"
    )
    modify_parser.add_argument(
        "--input-height", type=int, help="Input image height (for auto-center)"
    )
    ...
    batch_parser.add_argument(
        "-f", "--batch-file", required=True, help="JSON file with batch data"
    )
    batch_parser.add_argument(
        "-m",
        "--model",
        choices=SUPPORTED_MODELS,
        default="photon",
        help="Default model (can be overridden per image)",
    )
