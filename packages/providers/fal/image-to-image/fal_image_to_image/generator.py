...
class FALImageToImageGenerator:
        ...
        if api_key:
            ...
            if not api_key:
                ...
                if os.environ.get("CI") or os.environ.get("GITHUB_ACTIONS"):
                    ...
                    raise ValueError(
                        "FAL_KEY environment variable is not set. Please set it or provide api_key parameter."
                    )
            else:
    ...
    def modify_image(
        ...
        **kwargs,
        ...
        if model not in self.models:
            raise ValueError(
                f"Unsupported model: {model}. Supported models: {list(self.models.keys())}"
            )

        return self.models[model].generate(
            prompt=prompt, image_url=image_url, output_dir=output_dir, **kwargs
        )
    ...
    def upscale_image(
        ...
        kwargs = {"scale": scale, "enable_enhancement": enable_enhancement}
        if prompt:
    ...
    def batch_modify_images(
        ...
        **kwargs,
        ...
        print(
            f"🎨 Starting batch modification of {total_images} images with {model}..."
        )
