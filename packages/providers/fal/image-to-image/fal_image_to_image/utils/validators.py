...
from ..config.constants import (
    ...
    PHOTON_STRENGTH_RANGE,
    KONTEXT_INFERENCE_STEPS_RANGE,
    KONTEXT_GUIDANCE_SCALE_RANGE,
    SEEDEDIT_GUIDANCE_SCALE_RANGE,
...
def validate_aspect_ratio(aspect_ratio: str, model: str = "photon") -> str:
    ...
    if model in ["photon", "photon_base"]:
        if aspect_ratio not in ASPECT_RATIOS:
            raise ValueError(
                f"Unsupported aspect ratio for Photon models: {aspect_ratio}. Supported ratios: {ASPECT_RATIOS}"
            )
    elif model == "kontext":
        ...
        if aspect_ratio not in valid_modes:
            raise ValueError(
                f"Unsupported resolution mode for Kontext: {aspect_ratio}. Supported modes: {valid_modes}"
            )
    elif model == "kontext_multi":
        if aspect_ratio not in KONTEXT_MULTI_ASPECT_RATIOS:
            raise ValueError(
                f"Unsupported aspect ratio for Kontext Multi: {aspect_ratio}. Supported ratios: {KONTEXT_MULTI_ASPECT_RATIOS}"
            )
...
def validate_strength(strength: float) -> float:
    ...
    if not min_val <= strength <= max_val:
        raise ValueError(
            f"Strength must be between {min_val} and {max_val}, got: {strength}"
        )
    return strength
...
def validate_inference_steps(steps: int) -> int:
    ...
    if not min_val <= steps <= max_val:
        raise ValueError(
            f"Inference steps must be between {min_val} and {max_val}, got: {steps}"
        )
    return steps
...
def validate_guidance_scale(scale: float, model: str = "kontext") -> float:
    ...
    if not min_val <= scale <= max_val:
        raise ValueError(
            f"Guidance scale for {model} must be between {min_val} and {max_val}, got: {scale}"
        )
    return scale
...
def validate_num_images(num_images: int, max_images: int = 10) -> int:
    ...
    if not 1 <= num_images <= max_images:
        raise ValueError(
            f"Number of images must be between 1 and {max_images}, got: {num_images}"
        )
    return num_images
...
def validate_output_format(format_str: str) -> str:
    ...
    if format_str not in OUTPUT_FORMATS:
        raise ValueError(
            f"Output format must be one of {OUTPUT_FORMATS}, got: {format_str}"
        )
    return format_str
...
def validate_resolution(resolution: str) -> str:
    ...
    if resolution not in RESOLUTIONS:
        raise ValueError(
            f"Invalid resolution: {resolution}. Valid options: {RESOLUTIONS}"
        )
    return resolution


def validate_image_urls(
    image_urls: List[str], min_count: int = 1, max_count: int = 4
) -> List[str]:
    """
...
def validate_reframing_coordinates(
    ...
    if any(coord is None for coord in [x_start, y_start, x_end, y_end]):
        raise ValueError(
            "If any reframing coordinate is provided, all must be provided"
        )
...
def validate_grid_position(
    grid_position_x: Union[int, None], grid_position_y: Union[int, None]
) -> tuple:
    """
    ...
    if (grid_position_x is None) != (grid_position_y is None):
        raise ValueError(
            "Both grid_position_x and grid_position_y must be provided together"
        )
