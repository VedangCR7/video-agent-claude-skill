...
def print_cost_warning():
    ...
    response = (
        input("\n❓ Do you want to continue and incur charges? (yes/no): ")
        .strip()
        .lower()
    )
...
def test_clarity_with_generator():
    ...
    try:
        ...
        result2 = generator.upscale_local_image(
            image_path=image_path, scale=4, enable_enhancement=False
        )
        ...
        return {"test1": result1, "test2": result2, "model_info": info}
...
def main():
    ...
    if result.get("success"):
        ...
        if generator_results.get("test1", {}).get("success") and generator_results.get(
            "test2", {}
        ).get("success"):
    ...
    if os.path.exists(output_dir):
        output_files = [
            f for f in os.listdir(output_dir) if f.startswith("clarity_upscaled_")
        ]
        if output_files:
