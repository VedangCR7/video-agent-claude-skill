...
def test_replicate_generator():
    ...
    try:
        ...
        assert hasattr(ReplicateTextToImageModel, "SEEDREAM3"), (
            "❌ SEEDREAM3 model not found"
        )
        assert ReplicateTextToImageModel.SEEDREAM3.value == "bytedance/seedream-3", (
            "❌ Incorrect endpoint"
        )
        ...
        assert "SEEDREAM3" in all_models["available_models"], (
            "❌ Seedream-3 not in model list"
        )
...
def test_unified_generator():
    ...
    try:
        from unified_text_to_image_generator import (
        ...
        try:
            ...
            assert cheapest in available_models, (
                "❌ Cheapest model not in available models"
            )
            assert fastest in available_models, (
                "❌ Fastest model not in available models"
            )
            assert highest_quality in available_models, (
                "❌ Highest quality model not in available models"
            )

            print("✅ Optimal model selection works:")
