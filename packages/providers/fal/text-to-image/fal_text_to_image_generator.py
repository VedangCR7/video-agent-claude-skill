...
class FALTextToImageGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "FAL_KEY not found in environment variables or provided as parameter"
            )
    ...
    def validate_model(self, model: str) -> str:
        ...
        if model not in self.MODEL_ENDPOINTS:
            ...
            raise ValueError(
                f"Model '{model}' not supported. Available models: {available_models}"
            )
    ...
    def generate_image(
        ...
        **kwargs,
        ...
        payload = {"prompt": prompt, **default_params}
        ...
        try:
            ...
            if result and "images" in result and len(result["images"]) > 0:
                ...
                response = {
                    ...
                    "image_size": str(image_data.get("width", "unknown"))
                    + "x"
                    + str(image_data.get("height", "unknown"))
                ...
                if is_base64_url:
                    print(
                        f"📦 Received base64 image data ({len(raw_url) / 1024:.1f} KB)"
                    )
                else:
        ...
        try:
            ...
            if image_url.startswith("data:"):
                ...
                try:
                    ...
                    print(
                        f"✅ Image saved successfully! ({len(image_data) / 1024:.1f} KB)"
                    )
                    return filepath
    ...
    def batch_generate(
        ...
        **model_kwargs,
        ...
        if negative_prompt:
            ...
            print(
                f"❌ Negative prompt (for {', '.join(compatible_models)}): {negative_prompt}"
            )
        print(f"💰 Estimated cost: ~${len(models) * 0.015:.3f} (~$0.01-0.02 per image)")

        if not auto_confirm:
            confirm = input(
                "⚠️ This will generate multiple images and cost money. Continue? (y/N): "
            )
        ...
        for i, model in enumerate(models, 1):
            ...
            try:
                    ...
                    if download_images:
                        try:
                            ...
                            local_path = self.download_image(
                                result["image_url"], output_folder, filename
                            )
                ...
                results[model] = {"success": False, "error": str(e), "model": model}
    ...
    def compare_models(
        ...
        """
        print(
            "ℹ️ Using legacy compare_models method. Consider using batch_generate for more options."
        )
    ...
    def load_prompt_from_input(self, prompt_input: str) -> str:
        ...
        if (
            prompt_input.endswith((".txt", ".md", ".prompt"))
            or os.path.sep in prompt_input
            or os.path.exists(prompt_input)
        ):
    ...
    def generate_from_cli(
        ...
        **model_params,
        ...
        try:
            ...
            # Generate image
            result = self.generate_image(prompt=prompt, model=model, **valid_params)
    ...
    def _filter_model_params(
        ...
        valid_params = {
            ...
            "gpt_image_1_5": {"image_size", "num_images", "sync_mode"},
        }

        model_valid_params = valid_params.get(model, set())
        filtered = {
            k: v for k, v in params.items() if k in model_valid_params and v is not None
        }
