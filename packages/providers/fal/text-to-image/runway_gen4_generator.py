...
class RunwayGen4Generator:
    ...
    def calculate_cost(
        ...
        cost_per_image = (
            config["cost_1080p"] if resolution == "1080p" else config["cost_720p"]
        )
        return cost_per_image * num_images

    def _validate_reference_images(
        self, reference_images: Optional[List[str]], reference_tags: Optional[List[str]]
    ) -> bool:
        ...
        if reference_tags and len(reference_tags) != len(reference_images):
            raise ValueError(
                "Number of reference tags must match number of reference images"
            )

        return True

    def _prepare_reference_images(
        self, reference_images: Optional[List[str]]
    ) -> Optional[List[str]]:
        """
    ...
    def generate_image(
        ...
        try:
            ...
            output = replicate.run(model.value, input=input_args)
    ...
    def print_model_comparison(self):
        ...
        for model, config in self.MODEL_CONFIGS.items():
            ...
            print(
                f"💰 Cost: ${config['cost_720p']:.3f} (720p) / ${config['cost_1080p']:.3f} (1080p)"
            )
