            ...
            if downloaded_files:
                ...
                for file_path in downloaded_files:
                    if os.path.exists(file_path):
                        ...
                        print(
                            f"   🖼️  {os.path.basename(file_path)} ({file_size:,} bytes)"
                        )
...
Examples:
  ...
  python -m fal_text_to_image compare -p prompts/test.txt --save-json results.json
    ...
    setup_parser = subparsers.add_parser(
        "test-setup", help="Test environment setup (FREE)"
    )
    ...
    generate_parser.add_argument(
        "-m",
        "--model",
        choices=["imagen4", "seedream", "flux_schnell", "flux_dev"],
        required=True,
        help="Model to use",
    )
    generate_parser.add_argument(
        "-o", "--output-dir", help="Output directory (default: output)"
    )

    # Model-specific parameters
    generate_parser.add_argument(
        "--seed", type=int, help="Random seed (seedream, flux_dev)"
    )
    generate_parser.add_argument(
        "--num-inference-steps", type=int, help="Number of inference steps"
    )
    generate_parser.add_argument(
        "--guidance-scale",
        type=float,
        help="Guidance scale (imagen4, seedream, flux_dev)",
    )
    generate_parser.add_argument(
        "--image-size", help="Image size (e.g., landscape_4_3, square, portrait_16_9)"
    )
    generate_parser.add_argument(
        "--negative-prompt", help="Negative prompt (seedream, flux_dev only)"
    )
    generate_parser.add_argument(
        "--num-images", type=int, help="Number of images to generate (default: 1)"
    )
    generate_parser.add_argument("--save-json", help="Save full result as JSON")

    # Compare command
    compare_parser = subparsers.add_parser(
        "compare", help="Compare all models with same prompt"
    )
