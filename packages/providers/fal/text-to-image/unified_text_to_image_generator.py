...
try:
    from replicate_text_to_image_generator import (
        ReplicateTextToImageGenerator,
        ReplicateTextToImageModel,
    )
except ImportError:
    print(
        "❌ Replicate generator not found. Check replicate_text_to_image_generator.py"
    )
...
class UnifiedTextToImageGenerator:
    ...
    def __init__(
        ...
        if FALTextToImageGenerator:
            try:
                self.providers[Provider.FAL] = FALTextToImageGenerator(
                    api_key=fal_api_key
                )
                if verbose:
        ...
        if ReplicateTextToImageGenerator:
            try:
                self.providers[Provider.REPLICATE] = ReplicateTextToImageGenerator(
                    api_token=replicate_api_token, verbose=False
                )
                if verbose:
        ...
        if RunwayGen4Generator:
            try:
                self.runway_gen4 = RunwayGen4Generator(
                    api_token=replicate_api_token, verbose=False
                )
                if verbose:
        ...
        if not self.providers:
            ...
            if (
                ...
                or not any(
                    [os.environ.get("FAL_KEY"), os.environ.get("REPLICATE_API_TOKEN")]
                )
            ):
                if verbose:
                ...
                raise ValueError(
                    "No providers could be initialized. Check API keys and dependencies."
                )

        if verbose:
            print(
                f"🎨 Unified Text-to-Image Generator initialized with {len(self.providers)} provider(s)"
            )
        ...
        **kwargs,
        ...
        if provider:
            ...
            if provider != target_provider:
                raise ValueError(
                    f"Model '{model}' is only available on {target_provider.value}, not {provider.value}"
                )
        ...
        if target_provider == Provider.FAL:
            result = self.providers[Provider.FAL].generate_image(
                prompt=prompt, model=model_config["model_key"], **kwargs
            )
            ...
            if model == "gen4":
                ...
                result = self.runway_gen4.generate_image(
                    prompt=prompt, model=model_enum, **kwargs
                )
            else:
                ...
                model_enum = getattr(
                    ReplicateTextToImageModel, model_config["model_key"]
                )
    ...
    def _get_optimal_model(self, optimize_for: str) -> str:
        ...
        if optimize_for == "cost":
            ...
            return min(
                ...
                key=lambda m: available_models[m]["cost_per_image"],
            )
            ...
            return max(
                available_models.keys(),
                key=lambda m: quality_order.get(available_models[m]["quality"], 0),
            )
            ...
            return max(
                available_models.keys(),
                key=lambda m: speed_order.get(available_models[m]["speed"], 0),
            )
        else:
    ...
    def print_model_comparison(self):
        ...
        for provider_name, models in providers.items():
            ...
            for model_key, config in models:
                ...
                print(
                    f"   ⚡ Speed: {config['speed']} | 🎯 Quality: {config['quality']}"
                )
