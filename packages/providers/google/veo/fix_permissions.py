...
def enable_required_apis(project_id):
    ...
    for api in apis:
        run_command(
            f"gcloud services enable {api} --project={project_id}", f"Enabling {api}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fix Google Cloud Storage permissions for Veo"
    )
    parser.add_argument("--project-id", help="Google Cloud project ID")
    parser.add_argument("--bucket-name", help="GCS bucket name (without gs:// prefix)")
    parser.add_argument(
        "--bucket-uri", help="Full GCS bucket URI (e.g., gs://bucket/path/)"
    )
    ...
    if not project_id or not project_number:
        print(
            "❌ Could not get project information. Make sure you're authenticated with gcloud."
        )
        sys.exit(1)
