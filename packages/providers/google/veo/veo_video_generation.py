...
def generate_video_from_text(
    ...
    try:
        ...
        while not operation.done:
            ...
            print(
                f"Operation status: {operation.metadata.state if operation.metadata else 'Processing...'}"
            )
        ...
        if operation.response and operation.result.generated_videos:
            ...
            print(
                "Operation finished but no video URI found or an unknown error occurred."
            )
            return None
...
def generate_video_with_veo3_preview(
    project_id, prompt, output_bucket_path, location="us-central1"
):
    """
...
def download_gcs_file(gcs_uri: str, local_folder_path: str, project_id: str):
    ...
    try:
        ...
        if not file_name:  # Handle cases like "gs://bucket/folder/"
            print(
                f"Invalid GCS URI - URI points to a folder or has a trailing slash: {gcs_uri}"
            )
            return None
...
if __name__ == "__main__":
    ...
    # """
    veo3_prompt = (
        "A futuristic cityscape with flying vehicles and neon lights, cinematic style."
    )
    video_uri_veo3 = generate_video_with_veo3_preview(
        project_id=PROJECT_ID, prompt=veo3_prompt, output_bucket_path=OUTPUT_BUCKET_PATH
    )
