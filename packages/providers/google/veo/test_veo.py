...
class VeoTestSuite:
    ...
    def test_environment_setup(self):
        ...
        if not OUTPUT_BUCKET_PATH or OUTPUT_BUCKET_PATH == "gs://your-bucket/output/":
            self.log_test(
                "Bucket Configuration", False, "OUTPUT_BUCKET_PATH not configured"
            )
            return False
        ...
        try:
            ...
            if "not allowlisted" in error_msg.lower():
                self.log_test(
                    "Text-to-Video (Veo 3.0)",
                    False,
                    "Project not allowlisted for Veo 3.0",
                )
            else:
    ...
    def test_image_to_video_local(self):
        ...
        if not image_files:
            self.log_test(
                "Image-to-Video (Local)", False, "No images found in images directory"
            )
            return None
        ...
        test_prompt = (
            "The image comes to life with gentle, natural movements and soft lighting."
        )
    ...
    def test_image_to_video_no_prompt(self):
        ...
        if not images_dir.exists():
            self.log_test(
                "Image-to-Video (No Prompt)", False, "Images directory not found"
            )
            return None
        ...
        try:
            ...
            if video_uri:
                ...
                self.log_test(
                    "Image-to-Video (No Prompt)", False, "No video URI returned"
                )
                return None
        ...
        try:
            ...
            if local_path and os.path.exists(local_path):
                ...
                self.log_test(
                    "Download Functionality",
                    False,
                    "File not downloaded or doesn't exist",
                )
                return False
    ...
    def test_error_handling(self):
        ...
        try:
            video_uri = generate_video_from_text(
                project_id=PROJECT_ID, prompt="", output_bucket_path=OUTPUT_BUCKET_PATH
            )
            ...
            if video_uri is None:
                ...
                self.log_test(
                    "Error Handling (Empty Prompt)",
                    False,
                    "Should not generate video with empty prompt",
                )
        except Exception:
            self.log_test(
                "Error Handling (Empty Prompt)",
                True,
                "Correctly raised exception for empty prompt",
            )
        ...
        try:
            ...
            if video_uri is None:
                self.log_test(
                    ...
                    True,
                    "Correctly handled invalid image",
                )
            else:
                self.log_test(
                    "Error Handling (Invalid Image)",
                    False,
                    "Should not generate video with invalid image",
                )
        except Exception:
            self.log_test(
                "Error Handling (Invalid Image)",
                True,
                "Correctly raised exception for invalid image",
            )
    ...
    def print_summary(self):
        ...
        print(
            f"Success Rate: {(self.passed_tests / total_tests * 100):.1f}%"
            if total_tests > 0
            else "N/A"
        )
        ...
        successful_videos = [
            r for r in self.test_results if r["success"] and r["video_uri"]
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Google Veo Video Generation Test Suite"
    )
    ...
    parser.add_argument(
        "--image-only", action="store_true", help="Test only image-to-video"
    )
    parser.add_argument(
        "--text-only", action="store_true", help="Test only text-to-video"
    )
